...
public class SignalBarsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            textPaint.setTextSize(9 * density);
            canvas.drawText(signal.avgSnr > 0 ? signal.avgSnr + "dB" : "--", right + gap, midY - 2 * density, textPaint);
            textPaint.setColor(COLOR_TEXT_DIM);
            textPaint.setTextSize(7 * density);
            canvas.drawText(signal.satCount + (signal.satCount == 1 ? " sat" : " sats"), right + gap, midY + 9 * density, textPaint);
        }
    }
